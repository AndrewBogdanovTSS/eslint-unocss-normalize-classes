...
/**
 * The base every theme builds on - a small, ordinary project config.
 *
 ...
 *
 * The shortcuts here are not `scoped`: `card` is the same utilities in every
 * theme, so collapsing into it is safe anywhere, shared code included. Compare
 * `themes/<theme>/uno.config.ts`, where the same name means different things.
 */
